...
requires("1.52p")
suffix = "nd2";
suffixes = newArray("1sc","4etc","afi","aim","al3d","ali","am","amiramesh","apl","arf","avi","bip","bmp","btf","c01",
					"ch5","cif","cr2","crw","csv","dat","dcm","dib","dicom","dm2","dm3","dm4","dv","eps","epsi","exp",
					"fff","fits","flex","fli","frm","gel","gif","grey","gz","h5","hdr","hed","his","htd","hx","i2i",
					"ics","ids","im3","img","ims","inr","ipl","ipm","ipw","j2k","jp2","jpf","jpg","jpk","jpx","klb",
					"l2d","labels","lei","lif","liff","lim","map","mea","mnc","mng","mod","mrc","mrcs","mrw","msr",
					"mtb","naf","nd","nd2","ndpi","ndpis","nef","nhdr","nii","nrrd","obf","obsep","oib","oif","oir",
					"ome","par","pcoraw","pcx","pds","pic","png","pnl","ps","r3d","raw","rcpnl","rec","res","scn","sdt",
					"seq","sif","sld","spc","st","stk","stp","svs","sxm","tf2","tf8","tif","tiff","tnb","top","txt","v",
					"vff","vms","vsi","wpi","xdce","xml","xv");
...
{	
	...
	if(filename==0) {exit("No images found in\n"+dir);}
	...
	while(getBoolean("Current aggregate threshold: upper - "+lower+" lower - "+upper+"\nGet image and adjust threshold?","Get image","Next step"))
	{
	...
	while(getBoolean("Determine aggregate max & min size cutoff and perinuclear zone cutoff\nGet image to measure and display aggregate size\nand distance from nucleus","Get image","Next step"))
	{
		...
		waitForUser("Make note of distance to nucleus perimeter\nand aggregate size\nUse '+' and '-' keys to zoom");
		roiManager("reset");
		run("Clear Results");
		close("*");
	}
	...
	while(getBoolean("Optimize cell body processing\nGet image to show three levels of processing: 2.5, 5 & 7.5\nfor both segmentation and thresholding methods","Get image","Next step (settings)"))
	{
		...
		close("Dapi");
		run("Tile");
		waitForUser("Note strictness level and method from image title");
		close("*");
	}
	run("Clear Results");
}
...
Dialog.addNumber("Aggregate min size:", 0.1, 2, 4, "um^2");
Dialog.addToSameRow();
...
if(indexOf(filename,".")>0) 
{
	...
	{
		if(matches(filenamesuffix,suffixes[j]) || matches(filenamesuffix,suffix))
		{
			suffixmatch = true;
			j = lengthOf(suffixes);
		}
...
for(j=0;j<lengthOf(aggArea);j++) //print aggregate data to image data fiel
{
